...
export default function VoiceProfile() {
  ...
  if (loading) {
    return <div className="p-8 text-ink-400">Loading your Voice DNA...</div>
  }

  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-8 flex items-center gap-3">
        <Feather size={24} className="text-gold-400" />
        <h1 className="font-display text-display-md font-semibold">Voice DNA</h1>
      </div>

      {!profile?.voice_summary ? (
        <div className="bg-gold-800/30 border border-gold-700/50 rounded-lg p-6 text-gold-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
          ...
          <div className="lg:col-span-2 card p-6">
            ...
            <p className="text-ink-200 leading-relaxed whitespace-pre-line">{profile.voice_summary}</p>
          </div>
          ...
          <div className="card p-6 space-y-4">
            ...
            <div>
              <span className="text-xs text-ink-400 uppercase tracking-wide">Translation</span>
              <p className="text-ink-100 font-medium">{profile.preferred_translation || 'NKJV'}</p>
            </div>
          </div>
            ...
            <div className="lg:col-span-3 card p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {profile.signature_phrases.map((p) => (
                  <span key={p} className="text-sm bg-gold-800/40 border border-gold-700/50 text-gold-300 rounded-full px-3 py-1.5">
                    "{p}"
                  </span>
                ))}
              </div>
          ...
          {profile.anchor_scriptures && profile.anchor_scriptures.length > 0 && (
            <div className="lg:col-span-3 card p-6">
              <h2 className="font-display text-display-xs font-semibold mb-3 flex items-center gap-2">
                <BookOpen size={16} className="text-gold-400" /> Anchor Scriptures
              </h2>
              <div className="flex flex-wrap gap-3">
                {profile.anchor_scriptures.map((s) => (
                  <div key={s.ref} className="scripture-block py-2 px-3">
                    <span className="font-mono text-sm not-italic text-gold-300">{s.ref}</span>
                    {s.themes && s.themes.length > 0 && (
                      <div className="text-xs text-ink-400 mt-1 not-italic">{s.themes.join(', ')}</div>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="lg:col-span-3 card p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {profile.style_tags.map((t) => (
                  <span key={t} className="text-xs bg-ink-700 border border-ink-600 text-ink-200 rounded px-2.5 py-1 capitalize">
                    {t.replace(/_/g, ' ')}
                  </span>
                ))}
              </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <GitCommit size={20} className="text-gold-400" />
        ...
        {timeline.length === 0 ? (
          <div className="text-ink-500 text-sm italic">No voice versions yet.</div>
        ) : (
            ...
            {timeline.map((v, idx) => {
              ...
              return (
                <div key={v.id} className="relative">
                  <div className="absolute -left-8 top-1.5 w-6 h-6 rounded-full bg-gold-gradient flex items-center justify-center text-ink-950 text-xs font-bold">
                    v{v.version_number}
                  </div>
                  <div className="card p-4">
                    <div className="flex items-center justify-between mb-1">
                      <h3 className="font-medium text-ink-100">{triggerLabels[v.trigger] || v.trigger}</h3>
                      <span className="text-xs text-ink-400">
                        {new Date(v.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                      </span>
                    </div>
                    {v.change_summary && <p className="text-sm text-ink-300 mb-2">{v.change_summary}</p>}
