    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card p-6 mb-6 space-y-4 animate-fade-in-up">
          ...
          <div>
            <label className="block text-sm text-ink-300 mb-1.5">Story</label>
            <textarea
              required
              value={story}
          ...
          <div>
            <label className="block text-sm text-ink-300 mb-1.5">Themes (comma-separated)</label>
            <input
              value={themesInput}
          ...
          <button type="submit" className="btn-gold">Save to Vault</button>
        </form>
      )}
          ...
          {testimonies.map((t) => (
            <div key={t.id} className="card p-5">
              <div className="flex items-start justify-between mb-2">
                ...
                <button onClick={() => handleDelete(t.id)} className="text-ink-500 hover:text-red-400">
                  <Trash2 size={16} />
                </button>
              </div>
              <p className="text-ink-300 text-sm leading-relaxed line-clamp-3 mb-3">{t.story}</p>
              {t.themes && t.themes.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {t.themes.map((theme) => (
                    <span key={theme} className="text-xs bg-ink-700 text-ink-300 rounded-full px-2.5 py-0.5">
                      {theme}
                    </span>
                  ))}
                </div>
